...
export function ApplicationsTable({
  ...
  const handleMarkRejected = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    updateStatusMutation.mutate({ id, status: ApplicationStatus.Rejected });
  };
          ...
          <TableBody>
            {applications.map((app) => (
              ...
              >
                ...
                <TableCell>
                  <p className="text-foreground">{app.jobTitle}</p>
                </TableCell>
                <TableCell>
                  <Badge
                    variant="outline"
                ...
                <TableCell>
                  {app.nextEventDate && app.nextEventType !== undefined ? (
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                      <div>
                        <p className="text-foreground">{eventTypeLabels[app.nextEventType]}</p>
                        <p className="text-xs text-muted-foreground">
                          {format(new Date(app.nextEventDate), 'MMM d, h:mm a')}
                        </p>
                      </div>
                    </div>
                  ) : (
                ...
                <TableCell>
                  <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                    {app.currentStatus !== ApplicationStatus.Rejected && (
                      <AlertDialog>
                        ...
                        <AlertDialogContent onClick={e => e.stopPropagation()}>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will mark your application at {app.companyName} for {app.jobTitle} as rejected.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={(e) => handleMarkRejected(e, app.id)}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent onClick={e => e.stopPropagation()}>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This will permanently delete your application at {app.companyName}. This action cannot be undone.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancel</AlertDialogCancel>
